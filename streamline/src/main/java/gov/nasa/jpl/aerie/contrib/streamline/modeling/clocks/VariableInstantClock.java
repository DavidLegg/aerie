...
public record VariableInstantClock(Instant time, int multiplier) implements Dynamics<Instant, VariableInstantClock> {
    @Override
    ...
    public VariableInstantClock step(Duration t) {
        return new VariableInstantClock(addToInstant(time, t.times(multiplier)), multiplier);
    }
}
